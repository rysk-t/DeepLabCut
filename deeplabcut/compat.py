...
def train_network(
    ...
    Args:
        ...
        save_iters (optional): Only for the TensorFlow engine (for the PyTorch engine see the
            ``torch_kwargs``: you can use ``save_epochs``). This variable is actually set in
            ``pose_config.yaml``. However, you can overwrite it with this hack. Don't use this regularly,
            just if you are too lazy to dig out the ``pose_config.yaml`` file for the corresponding project.
            If ``None``, the value from there is used, otherwise it is overwritten! Defaults to None.
        max_iters (optional): Only for the TensorFlow engine (for the PyTorch engine see the
            ``torch_kwargs``: you can use ``epochs``). This variable is actually set in
            ``pose_config.yaml``. However, you can overwrite it with this hack. Don't use this regularly,
            just if you are too lazy to dig out the ``pose_config.yaml`` file for the corresponding project.
            If ``None``, the value from there is used, otherwise it is overwritten! Defaults to None.
        epochs (optional): Only for the PyTorch engine (equivalent to the `max_iters` parameter for the
            TensorFlow engine). The maximum number of epochs to train the model for. If None, the value will be read
            from the `pytorch_config.yaml` file. An epoch is a single pass through the training dataset, which means
            your model has seen each training image exactly once. So if you have 64 training images for your network,
            an epoch is 64 iterations with batch size 1 (or 32 iterations with batch size 2, 16 with batch size 4,
            etc.). Defaults to None.
        save_epochs (optional): Only for the PyTorch engine (equivalent to the `save_iters` parameter for the
            TensorFlow engine). The number of epochs between each snapshot save. If None, the value will be read from
            the `pytorch_config.yaml` file. Defaults to None.
        allow_growth (bool, optional): Only for the TensorFlow engine. For some smaller GPUs the memory issues happen.
            If ``True``, the memory allocator does not pre-allocate the entire specified GPU memory region, instead
        ...
        gputouse (optional): Only for the TensorFlow engine (for the PyTorch engine see the ``torch_kwargs``: you can
            use ``device``). Natural number indicating the number of your GPU (see number in nvidia-smi).
            If you do not have a GPU put None.
            See: https://nvidia.custhelp.com/app/answers/detail/a_id/3751/~/useful-nvidia-smi-queries. Defaults to None.
        autotune (bool, optional): Only for the TensorFlow engine. Property of TensorFlow, somehow faster if ``False``
            (as Eldar found out, see https://github.com/tensorflow/tensorflow/issues/13317). Defaults to False.
        keepdeconvweights (bool, optional): Also restores the weights of the deconvolution layers (and the backbone)
            when training from a snapshot. Note that if you change the number of bodyparts, you need to set this to
            false for re-training. Defaults to True.
        ...
        superanimal_name (str, optional): Only for the TensorFlow engine. For the PyTorch engine, you need to specify
            this through the ``weight_init`` when creating the training dataset. Specified if transfer learning with
            superanimal is desired. Defaults to "".
        superanimal_transfer_learning (bool, optional): Only for the TensorFlow engine. For the PyTorch engine, you
            need to specify this through the ``weight_init`` when creating the training dataset. If set true, the
            training is transfer learning (new decoding layer). If set false, and superanimal_name is True, then the
            training is fine-tuning (reusing the decoding layer). Defaults to False.
        engine (Engine, optional): The default behavior loads the engine for the shuffle from the metadata.
        ...
        detector_path (str | Path, optional): Only for the PyTorch engine. The path to the detector model snapshot to
            resume training from. Defaults to None.
        batch_size (int, optional): Only for the PyTorch engine. The batch size to use while training. Defaults to None.
        detector_batch_size (int, optional): Only for the PyTorch engine. The batch size to use while training the
            detector. Defaults to None.
        detector_epochs (int, optional): Only for the PyTorch engine. The number of epochs to train the detector for.
            Defaults to None.
        detector_save_epochs (int, optional): Only for the PyTorch engine. The number of epochs between each detector
            snapshot save. Defaults to None.
        pose_threshold (float, optional): Only for the PyTorch engine. Used for memory-replay. Pseudo-predictions with
            confidence lower than this threshold are discarded for memory-replay. Defaults to 0.1.
        pytorch_cfg_updates (dict, optional): A dictionary of updates to the pytorch config. The keys are the
            dot-separated paths to the values to update in the config. For example, to update the gpus to run the
            training on, you can use: ``pytorch_cfg_updates = {"runner.gpus": [0, 1, 2, 3]}``. Defaults to None.
...
def return_train_network_path(
    ...
    Args:
        ...
        engine (Engine, optional): The default behavior loads the engine for the shuffle from the metadata. You can
            overwrite this by passing the engine as an argument, but this should generally not be done. Defaults to
            None.
...
def evaluate_network(
    ...
    **torch_kwargs,
    ...
    Args:
        config (str | Path): Full path of the config.yaml file.
        shuffles (Sequence[int], optional): List of integers specifying the shuffle indices of the training dataset.
            Defaults to [1].
        ...
        plotting (bool or str, optional): Plots the predictions on the train and test images. If provided it must be
            either ``True``, ``False``, ``"bodypart"``, or ``"individual"``. Setting to ``True`` defaults as
            ``"bodypart"`` for multi-animal projects. If a detector is used, the predicted bounding boxes will also be
            plotted. Defaults to False.
        show_errors (bool, optional): Display train and test errors. Defaults to True.
        comparison_bodyparts (str or list, optional): The average error will be computed for those body parts only. The
            provided list has to be a subset of the defined body parts. Defaults to "all".
        gputouse (int or None, optional): Indicates the GPU to use (see number in ``nvidia-smi``). If you do not have a
            GPU put `None`.
            See: https://nvidia.custhelp.com/app/answers/detail/a_id/3751/~/useful-nvidia-smi-queries. Defaults to None.
        rescale (bool, optional): Evaluate the model at the ``'global_scale'`` variable (as set in the
            ``pose_config.yaml`` file for a particular project). I.e. every image will be resized according to that
            scale and prediction will be compared to the resized ground truth. The error will be reported in pixels at
            rescaled to the *original* size. I.e. For a [200,200] pixel image evaluated at ``global_scale=.5``, the
            predictions are calculated on [100,100] pixel images, compared to 1/2*ground truth and this error is then
            multiplied by 2!. The evaluation images are also shown for the original size! Defaults to False.
        modelprefix (str, optional): Directory containing the deeplabcut models to use when evaluating the network.
            By default, the models are assumed to exist in the project folder. Defaults to "".
        per_keypoint_evaluation (bool): Compute the train and test RMSE for each keypoint, and save the results to a
            {model_name}-keypoint-results.csv in the evaluation-results folder. Defaults to False.
        snapshots_to_evaluate (List[str], optional): List of snapshot names to evaluate (e.g. ["snapshot-5000",
            "snapshot-7500"]). Defaults to None.
        pcutoff (float | list[float] | dict[str, float] | None): Only for the PyTorch engine. For the TensorFlow engine,
            please set the pcutoff in the `config.yaml` file. The cutoff to use for computing evaluation metrics. When
            `None` (default), the cutoff will be loaded from the project config. If a list is provided, there should be
            one value for each bodypart and one value for each unique bodypart (if there are any). If a dict is
            provided, the keys should be bodyparts mapping to pcutoff values for each bodypart. Bodyparts that are not
            defined in the dict will have pcutoff set to 0.6. Defaults to None.
        engine (Engine, optional): The default behavior loads the engine for the shuffle from the metadata. You can
            overwrite this by passing the engine as an argument, but this should generally not be done. Defaults to
            None.
        torch_kwargs: You can add any keyword arguments for the deeplabcut.pose_estimation_pytorch evaluate_network
            function here. These arguments are passed to the downstream function. Available parameters are
            `snapshotindex`, which overrides the `snapshotindex` parameter in the project configuration file. For
            top-down models the `detector_snapshot_index` parameter can override the index of the detector to use for
            evaluation in the project configuration file.
...
def return_evaluate_network_data(
    ...
    Args:
        config (str | Path): Full path of the config.yaml file.
        shuffle (int): Shuffle index of the training dataset. Defaults to 0.
        trainingsetindex (int, optional): Integer specifying which TrainingsetFraction to use. By default the first
        ...
        rescale (bool): Evaluate the model at the 'global_scale' variable (as set in the test/pose_config.yaml file for
            ...
            size. I.e. For a [200,200] pixel image evaluated at global_scale=.5, the predictions are calculated on
            [100,100] pixel images, compared to 1/2*ground truth and this error is then multiplied by 2!. The
    ...
    Examples:
        If you do not want to plot:
            ...
            deeplabcut.return_evaluate_network_data("/analysis/project/reaching-task/config.yaml", shuffle=[1])
...
def analyze_videos(
    ...
    **torch_kwargs,
    ...
    Args:
        ...
        gputouse (int or None, optional): Only for the TensorFlow engine (for the PyTorch engine see the
            ``torch_kwargs``: you can use ``device``). Indicates the GPU to use (see number in ``nvidia-smi``).
            If you do not have a GPU put ``None``.
            See: https://nvidia.custhelp.com/app/answers/detail/a_id/3751/~/useful-nvidia-smi-queries. Defaults to None.
        ...
        destfolder (str | Path | None, optional): Destination folder for analysis data. If ``None``, uses the
            video path. Pass this folder for subsequent analysis too. Defaults to None.
        batch_size (int or None, optional): Currently not supported by the PyTorch engine. Batch size for inference;
            overwrites ``pose_cfg.yaml`` if set. Defaults to None.
        ...
        TFGPUinference (bool, optional): Only for the TensorFlow engine. Perform inference on GPU with TensorFlow code.
        ...
        use_openvino (str, optional): Only for the TensorFlow engine. Use "CPU" for inference if OpenVINO is available
            in the Python environment.
        engine (Engine, optional): The default behavior loads the engine for the shuffle from the metadata. You can
            overwrite this by passing the engine as an argument, but this should generally not be done. Defaults to
                None.
        torch_kwargs: Any extra parameters to pass to the PyTorch API, such as ``device`` which can be used to specify
            the
...
def create_tracking_dataset(
    ...
    Args:
        config (str | Path): Full path of the config.yaml file.
        videos (list[str | Path]): A list of strings containing the full paths to videos from which to create a tracking
            dataset, or a path to the directory where all the videos with same extension are
            stored.
        ...
        trainingsetindex (int, optional): Integer specifying which TrainingsetFraction to use. By default the first
            (note that TrainingFraction is a list in config.yaml). Defaults to 0.
        gputouse (int or None, optional): Only for the TensorFlow engine (for the PyTorch engine use ``device``).
            Indicates the GPU to use (see number in ``nvidia-smi``). If you do not have a GPU put ``None``.
            See: https://nvidia.custhelp.com/app/answers/detail/a_id/3751/~/useful-nvidia-smi-queries. Defaults to None.
        TFGPUinference (bool, optional): Only for the TensorFlow engine. Perform inference on GPU with TensorFlow code.
            Introduced in "Pretraining boosts out-of-domain robustness for pose estimation" by Alexander Mathis,
            Mert Yüksekgönül, Byron Rogers, Matthias Bethge, Mackenzie W. Mathis.
        ...
        robust_nframes (bool, optional): Evaluate a video's number of frames in a robust manner. This option is slower
            (as the whole video is read frame-by-frame), but does not rely on metadata, hence its robustness against
            file corruption. Defaults to False.
        n_triplets (int): The number of triplets to extract for the dataset. Defaults to 1000.
        engine (Engine, optional): The default behavior loads the engine for the shuffle from the metadata. You can
            overwrite this by passing the engine as an argument, but this should generally not be done. Defaults to
            None.
...
def analyze_images(
    ...
    **torch_kwargs,
    ...
    Args:
        ...
        images (str, Path, list[str], list[Path]): The image(s) to run inference on. Can be the path to an image, the
            path to a directory containing images, or a list of image paths or directories
            containing images.
        frame_type (string, optional): Filters the images to analyze to only the ones with the given suffix (e.g.
            setting `frame_type`=".png" will only analyze ".png" images).
            The default behavior analyzes all ".jpg", ".jpeg" and ".png" images.
        destfolder (str, Path, optional): The directory where the predictions will be stored. If None, the predictions
            will be
            stored in the same directory as the first image given in the `images` argument (if it's a directory, that
            directory will be used; if it's an image, the directory containing the image will be used).
        shuffle (int, optional): An integer specifying the shuffle with which to run image analysis.
        trainingsetindex (int, optional): Integer specifying which TrainingsetFraction to use. By default, the first one
            is used (note that TrainingFraction is a list in config.yaml).
        max_individuals (int, optional): The maximum number of individuals to detect in each image. Set to the number of
        ...
        device (str, optional): The CUDA device to use for training. If None, the device will be taken from the
            ``pytorch_config.yaml`` file. Examples: {"cpu", "cuda", "cuda:0", "cuda:1"}.
            For more information, see https://pytorch.org/docs/stable/notes/cuda.html
        snapshot_index (int, optional): Index (starting at 0) of the snapshot to use for image analysis. To evaluate the
            last one, use -1. Default uses the value set in the project config.
        detector_snapshot_index (int, optional): Only for Top-Down PyTorch models. If defined, uses the detector with
            the given index for pose estimation. To evaluate the last one, use -1. Default uses the value set in the
            project config.
        save_as_csv (bool, optional): Saves the predictions in a .csv file. If provided it must be either ``True`` or
            ``False``. Defaults to False.
        modelprefix (str, optional): Directory containing the deeplabcut models to use when running image analysis. By
            default, the models are assumed to exist in the project folder.
        plotting (bool, str): Plots the predictions made by the model on the analyzed images. Results will be stored in
            a folder named `LabeledImages_{scorer}`, where scorer is the name of the model used to analyze the images.
            This folder will be in the same directory as the file containing the predictions (either the given
            `destfolder`, or the folder containing the first image to analyze).
            If provided it must be either ``True``, ``False``, ``"bodypart"``, or ``"individual"``. Setting to ``True``
                defaults as ``"bodypart"`` for multi-animal projects. If a detector is used, the predicted bounding
                boxes will also be plotted. Defaults to False.
        pcutoff (float, optional): The cutoff score when plotting pose predictions. Must be None or in (0, 1). If None,
            the pcutoff is read from the project configuration file. Defaults to None.
        bbox_pcutoff (float, optional): The cutoff score when plotting bounding box predictions. Must be None or in (0,
            1). If None, it is read from the project configuration file. Defaults to None.
        plot_skeleton (bool): If a skeleton is defined in the project's config.yaml, whether to plot the skeleton
            connecting the predicted bodyparts on the images. Defaults to False.
        torch_kwargs: Any extra parameters to pass to the PyTorch API, such as ``ctd_conditions``.

    Returns:
        dict: A dictionary mapping image paths (as strings) to model predictions.
...
def analyze_time_lapse_frames(
    ...
    Args:
        config (str | Path): Full path of the config.yaml file.
        directory (str | Path): Full path to directory containing the frames that shall be analyzed.
        frametype (string, optional): Checks for the file extension of the frames. Only images with this extension are
            analyzed. Defaults to ``.png``.
        shuffle (int, optional): Shuffle index of the training dataset used for training the network. Defaults to 1.
        trainingsetindex (int, optional): Integer specifying which TrainingsetFraction to use. By default the first
            (note that TrainingFraction is a list in config.yaml).
...
def convert_detections2tracklets(
    ...
    Args:
        ...
        inferencecfg (optional): Configuration file for inference (assembly of individuals). Ideally should be obtained
            from cross validation (during evaluation). By default the parameters are loaded from inference_cfg.yaml, but
            these get_level_values can be overwritten. Defaults to None.
        calibrate (bool, optional): If True, use training data to calibrate the animal assembly procedure. This improves
            its robustness to wrong body part links, but requires very little missing data. Defaults to False.
        window_size (int, optional): Recurrent connections in the past `window_size` frames are prioritized during
            assembly. By default, no temporal coherence cost is added, and assembly is driven mainly by part affinity
            costs. Defaults to 0.
        identity_only (bool, optional): If True and animal identity was learned by the model, assembly and tracking rely
            exclusively on identity prediction. Defaults to False.
        track_method (string, optional): Specifies the tracker used to generate the pose estimation data. For multiple
            animals, must be either 'box', 'skeleton', or 'ellipse' and will be taken from the config.yaml file if none
            is given.
        engine (Engine, optional): The default behavior loads the engine for the shuffle from the metadata. You can
            overwrite this by passing the engine as an argument, but this should generally not be done. Defaults to
            None.
...
def extract_maps(
    ...
    Args:
        ...
        trainingsetindex (int, optional): Integer specifying which TrainingsetFraction to use. By default the first
            (note that TrainingFraction is a list in config.yaml). This variable can also be set to "all".
        gputouse (int or None, optional): For the TensorFlow engine (for the PyTorch engine see ``device``). Specifies
            the GPU to use (see number in ``nvidia-smi``). If you do not have a GPU put ``None``. See:
            https://nvidia.custhelp.com/app/answers/detail/a_id/3751/~/useful-nvidia-smi-queries. Defaults to None.
        device (str or None, optional): The CUDA device to use for training. If None, the device will be taken from the
        ...
        rescale (bool): Evaluate the model at the 'global_scale' variable (as set in the test/pose_config.yaml file for
            ...
            compared to the resized ground truth. The error will be reported in pixels at rescaled to the *original*
            size. I.e. For a [200,200] pixel image evaluated at global_scale=.5, the predictions are calculated on
            [100,100] pixel images, compared to 1/2*ground truth and this error is then multiplied by 2!. The evaluation
            images are also shown for the original size! Defaults to False.
        engine (Engine, optional): The default behavior loads the engine for the shuffle from the metadata. You can
            overwrite this by passing the engine as an argument, but this should generally not be done. Defaults to
            None.
...
def extract_save_all_maps(
    ...
    """Extracts the scoremap, location refinement field and part affinity field prediction of the model.

    The maps will be rescaled to the size of the input image and stored in the corresponding model folder in
    /evaluation-results.
    ...
    Args:
        ...
        all_paf_in_one (bool): By default, all part affinity fields are displayed on a single frame. If false,
            individual fields are shown on separate frames.
        gputouse (int or None, optional): For the TensorFlow engine (for the PyTorch engine see ``device``). Specifies
            the GPU to use (see number in ``nvidia-smi``). If you do not have a GPU put ``None``. See:
            https://nvidia.custhelp.com/app/answers/detail/a_id/3751/~/useful-nvidia-smi-queries. Defaults to None.
        device (str or None, optional): The CUDA device to use for training. If None, the device will be taken from the
        ...
        snapshot_index: Only for PyTorch models. Index (starting at 0) of the snapshot we want to extract maps with. To
            ...
            the project config.
        detector_snapshot_index: Only for TD PyTorch models. If defined, uses the detector with the given index for pose
            estimation. To extract maps for all detector snapshots, use "all". Default uses the value set in the project
            config.
        engine (Engine, optional): The default behavior loads the engine for the shuffle from the metadata. You can
            overwrite this by passing the engine as an argument, but this should generally not be done. Defaults to
            None.
...
def export_model(
    ...
    Args:
        cfg_path (str | Path): Path to the DLC Project config.yaml file.
        shuffle (int, optional): The shuffle of the model to export. Defaults to 1.
        trainingsetindex (int, optional): The index of the training fraction for the model you wish to export. Defaults
            to 1.
        snapshotindex (int, optional): The snapshot index for the weights you wish to export. If None, uses the
            snapshotindex as defined in 'config.yaml'. Defaults to None.
        iteration (int, optional): The model iteration (active learning loop) you wish to export. If None, the iteration
            listed in the config file is used.
        TFGPUinference (bool, optional): Use the tensorflow inference model? For inference using DeepLabCut-live, it is
            recommended to set TFGPUinference=False. Defaults to True.
        overwrite (bool, optional): If the model you wish to export has already been exported, whether to overwrite.
            Defaults to False.
        make_tar (bool, optional): Compress the exported directory to a tar file. This is necessary to export to the
            model zoo, but not for live inference. Defaults to True.
        wipepaths (bool, optional): Removes the actual path of your project and the init_weights from pose_cfg.
        without_detector (bool, optional): PyTorch engine only. Exports top-down models without the detector.
        engine (Engine, optional): The default behavior loads the engine for the shuffle from the metadata. You can
            overwrite this by passing the engine as an argument, but this should generally not be done. Defaults to
            None.
