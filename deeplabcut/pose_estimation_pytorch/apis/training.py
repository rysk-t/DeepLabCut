...
def train(
    ...
    if task == Task.DETECT:
        device = utils.resolve_detector_device(device, utils.detector_variant(run_config), for_training=True)
...
def train_network(
    ...
    if pose_task == Task.TOP_DOWN and loader.model_cfg["detector"]["train_settings"]["epochs"] > 0:
        ...
        if detector_run_config["device"] == "auto":
            # an explicit detector.device set in the model configuration is kept; otherwise the
            # detector follows the top-level device, or its own auto policy when that is "auto"
        ...
        train(
            ...
            task=Task.DETECT,
            device=None if device == "auto" else device,  # "auto" is resolved from the detector config
